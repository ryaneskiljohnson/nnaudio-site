        ...
        if (template) {
          ...
          const htmlContent = template.htmlContent ?? t.html_content;
            ...
            if (template.variables?.visual_elements && Array.isArray(template.variables.visual_elements)) {
              console.log('≡ƒÄ¿ Loading template with visual elements:', template.variables.visual_elements.length);
...
<body>
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      console.log('≡ƒÜÇ Sending template payload:', templatePayload);
      console.log('≡ƒôï Template data state:', templateData);
  ...
  const handleAudienceToggle = (audienceId: string, isChecked: boolean) => {
    console.log('≡ƒÄ» Audience toggle:', { audienceId, isChecked });
    if (isChecked) {
      // Add to included, remove from excluded if present - SIMPLIFIED LOGIC
      setTemplateData(prev => {
        const newData = {
          ...prev,
          audienceIds: [...prev.audienceIds, audienceId], // Γ£à Simple append like campaigns
          excludedAudienceIds: prev.excludedAudienceIds.filter(id => id !== audienceId)
        };
        console.log('≡ƒô¥ Updated template data (include):', newData);
        return newData;
      });
    } else {
      ...
      setTemplateData(prev => {
        ...
        console.log('≡ƒô¥ Updated template data (remove):', newData);
        return newData;
      });
    }
  };
  ...
  const handleAudienceExclude = (audienceId: string) => {
    ...
    if (isCurrentlyExcluded) {
      ...
      setTemplateData(prev => ({
        ...prev,
        audienceIds: prev.audienceIds.filter(id => id !== audienceId),
        excludedAudienceIds: [...prev.excludedAudienceIds, audienceId] // Γ£à Simple append like campaigns
      }));
    }
  };
              ...
              <FormGrid>
                <div>
                  <div style={{ background: 'rgba(255, 255, 255, 0.02)', padding: '1rem', borderRadius: '8px' }}>
                    ...
                    <p><strong>Status:</strong> {templateData.status === 'active' ? '≡ƒƒó Active' : '≡ƒƒí Draft'}</p>
              ...
              >
                ...
                <div style={{
                  ...
                  <h3 style={{
                    ...
                    ≡ƒôº {templateData.subject || 'Template Preview'}
                  </h3>
                ...
                <div style={{
                  ...
                  <div style={{
                    ...
                    Preview Mode: {previewDevice === 'mobile' ? '≡ƒô▒ Mobile (375px)' : '≡ƒûÑ∩╕Å Desktop (600px)'}
                  </div>
                  <div style={{
                    color: 'var(--text-secondary)',
                    fontSize: '0.9rem'
