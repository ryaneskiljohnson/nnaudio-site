...
/**
 * Catalog holds start 15% left of face-on. Shader srcU on the
 * camera meridian is `fract(0.5 + 2 * uPhase)`, so a 15% texture
 * shift is half that in wrap phase. Cymasphere and CymaSynth pass
 * 0 instead so those zooms stay centered.
 */
export const HERO_WRAP_ARRIVE_PHASE = wrapPhase(-0.15 / 2);
