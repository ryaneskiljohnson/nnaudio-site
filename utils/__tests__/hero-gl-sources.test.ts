...
describe("hero source contracts", () => {
  ...
  it("keys bodies from the camera so the visible side stays lit", () => {
    ...
    expect(scene).not.toContain("this.world.add(this.nebula)");
    expect(skybox).toContain("unityVoronoi");
    expect(skybox).toContain("hero-stars");
    expect(skybox).toContain("wisps");
    expect(skybox).toContain("sizePick");
    expect(skybox).toContain("seam");
    expect(skybox).toContain("wrapLon");
    expect(skybox).toContain("-camWorld / camDist");
    expect(skybox).toContain("length(vNdc)");
    expect(skybox).toContain("asin(clamp(eq.y");
    expect(skybox).toContain("if (seam > 0.0)");
    expect(skybox).toContain("exp(-pow(cells / radius");
    expect(skybox).not.toContain("twinkle");
    expect(skybox).not.toContain("uTime");
    expect(skybox).not.toContain("hero-aurora");
    expect(skybox).not.toContain("HERO_AURORA_INTENSITY");
    expect(skybox).not.toContain("AURORA_FRAG");
    expect(skybox).not.toContain("createHeroNebula");
    expect(skybox).not.toContain("float belt =");
    expect(skybox).not.toContain("hero-cloud-");
    expect(skybox).not.toContain("TorusGeometry");
    expect(skybox).not.toContain("0xf60f6d");
    expect(skybox).toContain("gl_Position = vec4(position.xy, 0.999, 1.0)");
    expect(skybox).toContain("uViewToLocal");
    expect(skybox).toContain("uSpin");
    expect(skybox).toContain("INV_WORLD");
    expect(scene).toContain("this.sunSpinPhase");
    expect(circuit).toContain("scene.poseSun(0)");
    ...
    expect(scene).toContain("orientSynthToRingPlate");
    expect(scene).toContain("synthOscDiskEulerRad");
    expect(scene).not.toContain("facingPhaseFromDir");
    expect(bodies).toContain("capMap: handle.kind === \"synth\"");
    expect(wrap).toContain("uCapMap");
    expect(wrap).toContain("atan(n.x, n.y)");
    expect(wrap).toContain("0.75 * PI");
    expect(wrap).toContain("blendW");
    expect(wrap).toContain("(-spoke)");
  ...
  it("keeps the still up until the sun wrap is on the GPU", () => {
    ...
    expect(ecosystem).toContain("reduceMotion ? <StaticHeroPoster />");
    expect(ecosystem).not.toContain("loading: () => <StaticHeroPoster />");
