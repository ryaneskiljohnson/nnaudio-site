...
const FEATURE_IMAGES = [
  {
    url: 'https://nnaud.io/wp-content/uploads/2024/01/Rompl-FX.webp',
    fileName: 'rompl-features-fx.webp',
...
async function downloadAndUploadImages() {
  console.log('=== Downloading Remaining Rompl Feature Images ===\n');
  ...
  // For "Intuitive Plugin Layout", use the GUI image since it shows the layout
  const guiFeature = featureMap.get('Intuitive Plugin Layout');
  if (guiFeature && !guiFeature.image_url && product.background_image_url) {
    guiFeature.image_url = product.background_image_url;
    console.log('✓ Assigned GUI image to "Intuitive Plugin Layout" feature\n');
  }
