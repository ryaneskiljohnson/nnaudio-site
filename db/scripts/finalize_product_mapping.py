#!/usr/bin/env python3
"""
@fileoverview Script to finalize product mapping with manual decisions
@module finalize_product_mapping

This script applies manual mapping decisions and handles bundle expansions.
"""
...
def load_auto_mapped():
    """
    @brief Load auto-mapped products
    @returns List of mapping dictionaries
    """
    script_dir = Path(__file__).parent
    exports_dir = script_dir.parent / 'exports'
    auto_file = exports_dir / 'product_mapping_auto.csv'
...
def get_supabase_product_by_name(name):
    """
    @brief Get Supabase product by name
...
def finalize_mapping():
    """
    @brief Create final product mapping with all decisions
    @returns None
    """
    # Start with auto-mapped products
    mappings = load_auto_mapped()
    
    # Manual confirmations from review
    manual_mappings = [
        {
            'woocommerce_name': '20 For 20 MIDI Bundle 1',
            'supabase_name': '20 For 20 MIDI Bundle',
            'match_type': 'manual_confirmed'
        },
        {
            'woocommerce_name': 'Modern Song Constructions',
            'supabase_name': 'Modern Song Constructions Bundle',
            'match_type': 'manual_confirmed'
        },
        {
            'woocommerce_name': 'Orchestra Bundle – Xmas 2023',
            'supabase_name': 'Orchestral Plugin Bundle',
            'match_type': 'manual_confirmed'
        },
        {
            'woocommerce_name': 'Curio – Texture Generator',
            'supabase_name': 'Curio',
            'match_type': 'manual_confirmed'
        },
        {
            'woocommerce_name': 'Obscura – Tortured Orchestral Box',
            'supabase_name': 'Obscura',
            'match_type': 'manual_confirmed'
        },
    ]
    ...
    bundle_expansions = [
        {
            'woocommerce_name': 'Relaunch Plugin Bundle',
            'expanded_products': ['Curves EQ', 'Digital Echoes Delay', 'SubFlux', 'DigitalDreamscape']
        },
        {
            'woocommerce_name': 'Selection Box Bundle – Xmas 2023',
            'expanded_products': ['Reiya', 'Evanescent Baby Grand Piano']
        },
    ]
    
    # Add manual mappings
    for manual in manual_mappings:
        product = get_supabase_product_by_name(manual['supabase_name'])
        if product:
            mappings.append({
                'woocommerce_name': manual['woocommerce_name'],
                'supabase_id': product['id'],
                'supabase_name': product['name'],
                'supabase_status': product['status'],
                'match_type': manual['match_type']
            })
    
    # Write final mapping
    script_dir = Path(__file__).parent
    exports_dir = script_dir.parent / 'exports'
    final_file = exports_dir / 'product_mapping_final.csv'
    ...
    with open(final_file, 'w', encoding='utf-8', newline='') as f:
        ...
        writer.writeheader()
        writer.writerows(sorted(mappings, key=lambda x: x['woocommerce_name']))
    
    print(f"✓ Final mapping created: {final_file}")
    print(f"  Total mapped products: {len(mappings)}")
    
    # Write bundle expansions
    bundle_file = exports_dir / 'product_mapping_bundles.json'
    
    # Resolve bundle product IDs
    resolved_bundles = []
    for bundle in bundle_expansions:
        expanded = []
        for product_name in bundle['expanded_products']:
            product = get_supabase_product_by_name(product_name)
            if product:
                expanded.append({
                    'id': product['id'],
                    'name': product['name'],
        ...
        resolved_bundles.append({
            'woocommerce_name': bundle['woocommerce_name'],
            'expanded_products': expanded
        })
    
    with open(bundle_file, 'w', encoding='utf-8') as f:
        json.dump(resolved_bundles, f, indent=2)
    
    print(f"✓ Bundle expansions saved: {bundle_file}")
    
    # Products to skip (not migrate)
    skipped = [
        'Blended Bundle',
        'Lutes Of The World Bundle – PLUGINOMAT Deal',
        'Nice Pipes Bundle – Xmas 2023',
        'TestProd',
        'Toybox-Retro',
        'VSTAlarm FX Bundle',
        'VSTAlarm Summer Sounds Bundle',
        'Valentine MIDI Bundle + Free Plugin 2024'
    ]
    
    skip_file = exports_dir / 'product_mapping_skipped.txt'
    with open(skip_file, 'w', encoding='utf-8') as f:
        f.write("# Products NOT to migrate (excluded from mapping)\n\n")
        for product in sorted(skipped):
            f.write(f"- {product}\n")
    
    print(f"✓ Skipped products documented: {skip_file}")


if __name__ == '__main__':
    finalize_mapping()
    print("\n✓ Product mapping finalized!")
